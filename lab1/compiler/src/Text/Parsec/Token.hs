...
data GenLanguageDef s u m
    ...
    commentStart   :: ParsecT s u m String,
    commentStartStr :: String,

    -- | Describes the end of a block comment. Use the empty string if the
    -- language doesn't support block comments. For example \"*\/\".

    commentEnd     :: ParsecT s u m String,
...
makeTokenParser languageDef
    ...
    whiteSpace
        ...
        where
          noMulti = null (commentStartStr languageDef)
    ...
    multiLineComment =
        do { try (commentStart languageDef)
           ; inComment
           }
    ...
    inCommentMulti
        =   do{ try (commentEnd languageDef) ; return () }
        <|> do{ multiLineComment                     ; inCommentMulti }
        <|> do{ skipMany1 (noneOf startEnd)          ; inCommentMulti }
        <|> do{ oneOf startEnd                       ; inCommentMulti }
        <?> "end of comment"
        where
          startEnd   = nub (commentEndStr languageDef ++ commentStartStr languageDef)
